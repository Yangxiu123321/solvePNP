...
PNPSolver::PNPSolver()
{
	...
	Points3D.push_back(cv::Point3f(0,173.f,0));
	Points3D.push_back(cv::Point3f(173.f,0,0));
	Points3D.push_back(cv::Point3f(173.f,173.f,0));
}
...
int PNPSolver::Solve(METHOD method)
{
	...
	/*******************解决PNP问题*********************/
	//有三种方法求解，从世界坐标系到相机坐标系的旋转向量与平移矩阵
	solvePnP(Points3D, Points2D, camera_matrix, distortion, rvec, tvec, false, method);	//实测迭代法似乎只能用共面特征点求位置
...
int PNPSolver::applySolvePNP(vector<cv::Point2f> targetPoints)
{
	if(targetPoints.size() != 4)
	{
       return 0;
	}
	Mat paintBoard(Size(640,480),CV_8UC3,Scalar(255,255,255));
	// 值传递
	Points2D.assign(targetPoints.begin(),targetPoints.end());
	// 解位姿
	Solve(METHOD::CV_P3P);
	// 验证
	vector<cv::Point3f> r;
	r.push_back(cv::Point3f(0,0,105));

	vector<cv::Point2f> ps;
	ps = WordFrame2ImageFrame(r);
	for(int i = 0;i < ps.size();i++)
	{
		circle(paintBoard,ps[i],5,Scalar(0,255,255),-1);
	}
    circle(paintBoard,Point2f(200,100),5,Scalar(0,255,255),-1);
	// 输出位姿信息
	// std::cout << "c";
	std::cout << Oc.x << " "<< Oc.y <<" " << Oc.z << std::endl;
	// std::cout << "w";
	//   std::cout << Ow.x << " " << Ow.y << " " << Ow.z << std::endl;
	// std::cout << Theta_W2C.x << " " << Theta_W2C.y << " " << Theta_W2C.z << std::endl;
	imshow("paint",paintBoard);
}